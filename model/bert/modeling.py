...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, head_mask=None):
        ...
        # ###############
        # col_attention_mask = nn.Tanh()(attention_mask.permute(0, 1, 3, 2).contiguous()+10000.0)
        # ###############
...
class BertModelWithoutEmbeddings(BertPreTrainedModel):
    def __init__(self, config, output_attentions=False, keep_multihead_output=False):
        super(BertModelWithoutEmbeddings, self).__init__(config)
        self.output_attentions = output_attentions
        self.encoder = BertEncoder(config, output_attentions=output_attentions,
                                   keep_multihead_output=keep_multihead_output)
        self.pooler = BertPooler(config)
    ...
    def reshape_head_mask(self, head_mask):
        """ 1.0 in head_mask indicate we keep the head
            attention_probs has shape bsz x n_heads x N x N
            input head_mask has shape [num_heads] or [num_hidden_layers x num_heads]
            and head_mask is converted to shape [num_hidden_layers x batch x num_heads x seq_length x seq_length]
            Return: reshaped head mask
        """
        if head_mask.dim() == 1:
            head_mask = head_mask.unsqueeze(0).unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
            head_mask = head_mask.expand_as(self.config.num_hidden_layers, -1, -1, -1, -1)
    ...
    def forward(self, inputs, token_type_ids=None, attention_mask=None,
        ...
        # Prepare head mask if needed
        if head_mask is not None:
            head_mask = self.reshape_head_mask(head_mask)
        else:
            head_mask = [None] * self.config.num_hidden_layers

        encoded_layers = self.encoder(inputs,
                                      extended_attention_mask,
                                      output_all_encoded_layers=output_all_encoded_layers,
                                      head_mask=head_mask)
        if self.output_attentions:
...
class BertLstmForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, head_mask=None):
        ...
        """########################################"""
        pooled_output, _ = self.rnn(sequence_output)
